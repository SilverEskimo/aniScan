...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$get_all_users_query = "SELECT id, firstName, lastName, hasChipScanner, agreeToMessage, latitude, longitude FROM users WHERE id != '".$userId."'";
						...
						if ($distance_json->status == 'OK') {
							...
							if ($distanceValue < 5000 && $agreeToMessage == 1 && $hasChipScanner == 1) {
								...
								if (!$connection->query($insert_new_notification)) {
									...
									$phone_query = "SELECT phone FROM slava_db_new.users WHERE id =?";
									$text_query ="היי $firstName, יש במרחק $distanceText ממך בקשה לסיוע מ$loggedInUserFirstName $loggedInUserLastName! לחץ לפרטים נוספים";
									$stmt = $connection ->prepare($phone_query);
									$stmt -> bind_param("s", $currUserId);
									$stmt -> execute();
                                                                        $phone_result = $stmt ->get_result();
									$phoneFromUsers = $phone_result -> fetch_assoc();
									
									//$stmtt = $connection ->prepare($text_query);
									//$stmtt -> bind_param("s", $currUserId);
									//$stmtt -> execute();
									//$text_result = $stmtt -> get_result();
									//$textFromNotif = $text_result -> fetch_assoc();
									
									//$currText = $textFromNotif['text'];
									$currPhone = $phoneFromUsers['phone'];
									$phoneWithoutPref =  substr($currPhone,1);
								        $persMsg = "בדוק את ההתראות בפרופיל האישי";
									$urlToAdd = " https://ani-scan.com" . PHP_EOL . $persMsg;
									$currText = $text_query . $urlToAdd;
									$ch1 = curl_init('https://textbelt.com/text');
									$data = array(
									'phone'=> '+972' .$phoneWithoutPref,
									'message' => ''.$currText.'',
