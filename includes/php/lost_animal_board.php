...
<?php
	...
	$fbUrl = "https://";
	$fbUrl .= $_SERVER['HTTP_HOST'];
	$fbUrl .= $_SERVER['REQUEST_URI'];
	$fbUrl .= "?id=";
...
<head>
    <meta charset="utf-8">
    <title>Ani-Scan - Lost Animals Board</title>
...
<body>
	...
	<br>
	<div>
		<button class="pretty-button" onclick="location.href = '/includes/php/lost_animal_form.php';">הוספת אבידה</button>
	</div>
	<br>
	...
	<div class="container">
        <div class="board-page-content">
			<?php
				...
				$animals_query = "SELECT a.*, u.firstName as firstName, u.lastName as lastName, u.phone as phone FROM animals as a JOIN users as u on (a.created_user_id = u.id) WHERE isFound = 0 ORDER BY date_created DESC";
				...
				if ($animals_result->num_rows > 0) {
					while ($row = $animals_result->fetch_assoc()) {
						...
						if ($answers_result->num_rows > 0) {
							while ($answers_result_row = $answers_result-> fetch_assoc()) {
								if ($answers_result_row["animal-id"] == $animalId) {
									...
									echo '
									<div class="answer-content" dir="rtl">
						...
						echo '</div>
								<form action="lost_animal_board.php" method="post">
